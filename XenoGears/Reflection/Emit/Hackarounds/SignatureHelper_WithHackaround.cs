...
namespace XenoGears.Reflection.Emit.Hackarounds
{
    ...
    [DebuggerNonUserCode]
    public sealed class SignatureHelper_WithHackaround : _SignatureHelper
    {
        ...
        private SignatureHelper_WithHackaround(Module mod, int callingConvention)
        {
            this.Init(mod, callingConvention);
        }

        private SignatureHelper_WithHackaround(Module mod, Type type)
        {
            this.Init(mod);
            this.AddOneArgTypeHelper(type);
        }

        private SignatureHelper_WithHackaround(Module mod, int callingConvention, Type returnType, Type[] requiredCustomModifiers, Type[] optionalCustomModifiers) : this(mod, callingConvention, 0, returnType, requiredCustomModifiers, optionalCustomModifiers)
        {
        }

        private SignatureHelper_WithHackaround(Module mod, int callingConvention, int cGenericParameters, Type returnType, Type[] requiredCustomModifiers, Type[] optionalCustomModifiers)
        {
            this.Init(mod, callingConvention, cGenericParameters);
            if (callingConvention == 6)
            {
                throw new ArgumentException("Argument_BadFieldSig");
            }
        ...
        public void AddArgument(Type argument, Type[] requiredCustomModifiers, Type[] optionalCustomModifiers)
        {
            if (this.m_sigDone)
            {
                throw new ArgumentException("Argument_SigIsFinalized");
            }
            if (argument == null)
            {
                throw new ArgumentNullException("argument");
            ...
            if (requiredCustomModifiers != null)
            {
                for (int i = 0; i < requiredCustomModifiers.Length; i++)
                {
                    ...
                    {
                        throw new ArgumentException("Argument_ArraysInvalid", "requiredCustomModifiers");
                    }
                    if (type.ContainsGenericParameters)
                    {
                        throw new ArgumentException("Argument_GenericsInvalid", "requiredCustomModifiers");
                    }
                }
            }
            if (optionalCustomModifiers != null)
            {
                for (int j = 0; j < optionalCustomModifiers.Length; j++)
                {
                    ...
                    {
                        throw new ArgumentException("Argument_ArraysInvalid", "optionalCustomModifiers");
                    }
                    if (type2.ContainsGenericParameters)
                    {
                        throw new ArgumentException("Argument_GenericsInvalid", "optionalCustomModifiers");
                    }
                }
            }
            this.IncrementArgCounts();
        ...
        public void AddArguments(Type[] arguments, Type[][] requiredCustomModifiers, Type[][] optionalCustomModifiers)
        {
            ...
            {
                throw new ArgumentException("Argument_MismatchedArrays", "requiredCustomModifiers");
            }
            if ((optionalCustomModifiers != null) && ((arguments == null) || (optionalCustomModifiers.Length != arguments.Length)))
            {
                throw new ArgumentException("Argument_MismatchedArrays", "optionalCustomModifiers");
            }
            if (arguments != null)
            {
        ...
        private void AddData(int data)
        {
            ...
            else
            {
                if (data > 0x1fffffff)
                {
                    throw new ArgumentException("Argument_LargeInteger");
                }
        ...
        private void AddOneArgTypeHelperWorker(Type clsArgument, bool lastWasGenericInst)
        {
            ...
            {
                TypeToken token2;
                var enumBuilder = ((EnumBuilder) clsArgument);
                TypeBuilder typeBuilder = enumBuilder.GetSlot<TypeBuilder>("m_typeBuilder").Value;
            ...
            {
                var isSzArray = ((Object)clsArgument).GetSlot<bool>("IsSzArray").Value;
                if (isSzArray)
                {
                ...
                {
                    this.AddElementType(0x14);
                    this.AddOneArgTypeHelper(clsArgument.GetElementType());
                    this.AddData(clsArgument.GetArrayRank());
                    this.AddData(0);

                    // the line commented below is a bug
                    // see http://xeno-by.livejournal.com/14638.html for more details
//                    this.AddData(0);

                    var rank = clsArgument.GetArrayRank();
                    this.AddData(rank);
                    rank.TimesDo(() => this.AddData(0));
                }
            }
            else
            {
        ...
        {
            if (!(obj is SignatureHelper_WithHackaround))
            {
                return false;
            }
            SignatureHelper_WithHackaround helper = (SignatureHelper_WithHackaround) obj;
        ...
        internal static int GetCorElementTypeFromClass(Type cls)
        {
            var getCorElementTypeFromClass = typeof(System.Reflection.Emit.SignatureHelper)
                .GetFunc<_<Type>, int>("GetCorElementTypeFromClass");
            return getCorElementTypeFromClass(cls);
        }

        public static SignatureHelper_WithHackaround GetFieldSigHelper(Module mod)
        {
            return new SignatureHelper_WithHackaround(mod, 6);
        }
        ...
        public static SignatureHelper_WithHackaround GetLocalVarSigHelper()
        {
            return GetLocalVarSigHelper(null);
        }

        public static SignatureHelper_WithHackaround GetLocalVarSigHelper(Module mod)
        {
            return new SignatureHelper_WithHackaround(mod, 7);
        }

        public static SignatureHelper_WithHackaround GetMethodSigHelper(CallingConventions callingConvention, Type returnType)
        {
            return GetMethodSigHelper(null, callingConvention, returnType);
        }

        public static SignatureHelper_WithHackaround GetMethodSigHelper(CallingConvention unmanagedCallingConvention, Type returnType)
        {
            return GetMethodSigHelper(null, unmanagedCallingConvention, returnType);
        }

        public static SignatureHelper_WithHackaround GetMethodSigHelper(Module mod, CallingConventions callingConvention, Type returnType)
        {
            return GetMethodSigHelper(mod, callingConvention, returnType, null, null, null, null, null);
        }

        public static SignatureHelper_WithHackaround GetMethodSigHelper(Module mod, CallingConvention unmanagedCallConv, Type returnType)
        {
            int num;
            if (returnType == null)
            {
            ...
            else
            {
                ...
                {
                    throw new ArgumentException("Argument_UnknownUnmanagedCallConv", "unmanagedCallConv");
                }
                num = 4;
            }
            return new SignatureHelper_WithHackaround(mod, num, returnType, null, null);
        }

        public static SignatureHelper_WithHackaround GetMethodSigHelper(Module mod, Type returnType, Type[] parameterTypes)
        {
            return GetMethodSigHelper(mod, CallingConventions.Standard, returnType, null, null, parameterTypes, null, null);
        }

        public static SignatureHelper_WithHackaround GetMethodSigHelper(Module mod, CallingConventions callingConvention, Type returnType, int cGenericParam)
        {
            return GetMethodSigHelper(mod, callingConvention, cGenericParam, returnType, null, null, null, null, null);
        }

        public static SignatureHelper_WithHackaround GetMethodSigHelper(Module scope, CallingConventions callingConvention, Type returnType, Type[] requiredReturnTypeCustomModifiers, Type[] optionalReturnTypeCustomModifiers, Type[] parameterTypes, Type[][] requiredParameterTypeCustomModifiers, Type[][] optionalParameterTypeCustomModifiers)
        {
        ...
        public static SignatureHelper_WithHackaround GetMethodSigHelper(Module scope, CallingConventions callingConvention, int cGenericParam, Type returnType, Type[] requiredReturnTypeCustomModifiers, Type[] optionalReturnTypeCustomModifiers, Type[] parameterTypes, Type[][] requiredParameterTypeCustomModifiers, Type[][] optionalParameterTypeCustomModifiers)
        {
            if (returnType == null)
            {
                returnType = typeof(void);
            ...
            SignatureHelper_WithHackaround helper = new SignatureHelper_WithHackaround(scope, num, cGenericParam, returnType, requiredReturnTypeCustomModifiers, optionalReturnTypeCustomModifiers);
            helper.AddArguments(parameterTypes, requiredParameterTypeCustomModifiers, optionalParameterTypeCustomModifiers);
            return helper;
        }

        public static SignatureHelper_WithHackaround GetMethodSpecSigHelper(Module scope, Type[] inst)
        {
            SignatureHelper_WithHackaround helper = new SignatureHelper_WithHackaround(scope, 10);
        ...
        public static SignatureHelper_WithHackaround GetPropertySigHelper(Module mod, Type returnType, Type[] parameterTypes)
        {
            return GetPropertySigHelper(mod, returnType, null, null, parameterTypes, null, null);
        }

        public static SignatureHelper_WithHackaround GetPropertySigHelper(Module mod, Type returnType, Type[] requiredReturnTypeCustomModifiers, Type[] optionalReturnTypeCustomModifiers, Type[] parameterTypes, Type[][] requiredParameterTypeCustomModifiers, Type[][] optionalParameterTypeCustomModifiers)
        {
        ...
        public static SignatureHelper_WithHackaround GetPropertySigHelper(Module mod, CallingConventions callingConvention, Type returnType, Type[] requiredReturnTypeCustomModifiers, Type[] optionalReturnTypeCustomModifiers, Type[] parameterTypes, Type[][] requiredParameterTypeCustomModifiers, Type[][] optionalParameterTypeCustomModifiers)
        {
            if (returnType == null)
            {
                returnType = typeof(void);
            ...
            SignatureHelper_WithHackaround helper = new SignatureHelper_WithHackaround(mod, num, returnType, requiredReturnTypeCustomModifiers, optionalReturnTypeCustomModifiers);
            helper.AddArguments(parameterTypes, requiredParameterTypeCustomModifiers, optionalParameterTypeCustomModifiers);
            return helper;
        }
        ...
        public byte[] GetSignature(bool appendEndOfSig)
        {
            if (!this.m_sigDone)
            {
                if (appendEndOfSig)
        ...
        public static SignatureHelper_WithHackaround GetTypeSigToken(Module mod, Type type)
        {
            if (mod == null)
            {
            ...
            return new SignatureHelper_WithHackaround(mod, type);
        }
        ...
        private void Init(Module mod)
        {
            ...
            {
                throw new ArgumentException("NotSupported_MustBeModuleBuilder");
            }
        }
        ...
        private unsafe void InternalAddRuntimeType(Type type)
        {
            ...
            var getTypeHandleInternal = type.GetFunc<RuntimeTypeHandle>("GetTypeHandleInternal");
            void* voidPtr = (void*) getTypeHandleInternal().Value;
            if (sizeof(void*) == 4)
            {
        ...
        public byte[] InternalGetSignature(out int length)
        {
            if (!this.m_sigDone)
            {
                this.m_sigDone = true;
        ...
        public byte[] InternalGetSignatureArray()
        {
        ...
        public static bool IsSimpleType(int type)
        {
